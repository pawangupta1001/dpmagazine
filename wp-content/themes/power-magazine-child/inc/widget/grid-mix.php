<?php
/**
 * Grid Mix Style Widget.
 *
 * @package Power_Magazine
 */


function power_magazine_action_grid_mix() {

  register_widget( 'Power_magazine_Grid_Mix' );
  
}
add_action( 'widgets_init', 'power_magazine_action_grid_mix' );
class Power_magazine_Grid_Mix extends WP_Widget{ 

	function __construct() {
		global $control_ops;
		$widget_ops = array(
		  'classname'   => 'business-section',
		  'description' => esc_html__( 'Add Widget to Display Grid Mix Style Widget.', 'power-magazine' )
		);
		parent::__construct( 'power_magazine_grid_mix',esc_html__( 'PM: Grid Mix Style', 'power-magazine' ), $widget_ops, $control_ops );
	}
            ...
            <!--<div class="business-wrap">-->
                <?php while ( $the_query->have_posts() ) : $the_query->the_post(); $count++; ?>
                    <article class="featured-post post hentry category-featured">
                        <div class="heading">
                            <header class="entry-header">
                                <h3 class="entry-title">Health</h3>                
                            </header>
                        </div>
                    	<?php if ( has_post_thumbnail() ){ ?>
	                        <figure class="featured-post-image">
            ...
            <!--<div class="business-wrap">-->
                <?php while ( $the_query->have_posts() ) : $the_query->the_post(); $count++; ?>
                    <article class="featured-post post hentry category-featured">
                        <div class="heading">
                            <header class="entry-header">
                                <h3 class="entry-title">Lifestyle</h3>                
                            </header>
                        </div>
