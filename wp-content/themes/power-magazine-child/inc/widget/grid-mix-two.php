<?php
/**
 * Grid Mix Two Style Widget.
 *
 * @package Power_Magazine
 */


function power_magazine_action_grid_mix_two() {

  register_widget( 'Power_magazine_Grid_Mix_Two' );

}
add_action( 'widgets_init', 'power_magazine_action_grid_mix_two' );
class Power_magazine_Grid_Mix_Two extends WP_Widget{

    function __construct() {
        global $control_ops;
        $widget_ops = array(
          'classname'   => 'business-section',
          'description' => esc_html__( 'Add Widget to Display Grid Mix Two Style Widget.', 'power-magazine' )
        );
        parent::__construct( 'power_magazine_grid_mix_two',esc_html__( 'PM: Grid Mix Two Style', 'power-magazine' ), $widget_ops, $control_ops );
    }
    ...
    function widget( $args, $instance ) {
        ...
        extract( $args );
        $title              = ( ! empty( $inGrstance['title'] ) ) ? esc_html($instance['title']) : '';
            ...
            <!--<div class="business-wrap">-->
                <?php while ( $the_query->have_posts() ) : $the_query->the_post(); $count++; ?>
                    <article class="featured-post post hentry category-featured">
                        <div class="heading">
                            <header class="entry-header">
                                <h3 class="entry-title">Society</h3>
                            </header>
                        </div>
            ...
            <!--<div class="business-wrap">-->
                <?php while ( $the_query->have_posts() ) : $the_query->the_post(); $count++; ?>
                    <article class="featured-post post hentry category-featured">
                        <div class="heading">
                            <header class="entry-header">
                                <h3 class="entry-title">Personal Problems</h3>
                            </header>
                        </div>
